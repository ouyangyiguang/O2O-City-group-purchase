			...
			<scroll-view class="msg-list" scroll-y="true" :scroll-with-animation="scrollAnimation" :scroll-top="scrollTop" :scroll-into-view="scrollToView" @scrolltoupper="loadHistory" upper-threshold="50">
				...
				<view class="row" v-for="(row,index) in msgList" :key="index" :id="'msg'+row.msg.id">
					...
					<block v-if="row.type=='user'">
						<!-- 自己发出的消息 -->
						<view class="my" v-if="row.msg.userinfo.uid==user.userInfo.users_id">
							<!-- 左-消息 -->
							<view class="left">
								<!-- 文字消息 -->
						...
						<!-- 别人发出的消息 -->
						<view class="other" v-if="row.msg.userinfo.uid!=user.userInfo.users_id">
							<!-- 左-头像 -->
							<view class="left" >
	...
	onLoad(option) {
		...
		this.roomid=query.messageId;
				this.$api('chat.create_roomgroup', {
					...
					form_uid:this.user.userInfo.users_id,
					addr_id:this.user.wareRoom.addr_id
				}).then(res => {
						uni.setNavigationBarTitle({title: res.data.roomname});
						this.is_cun=res.data.is_cun;
				}).catch((e)=>{});
							if (query.messageId!=''){
								if (this.roomid==""){
		...
		getMessageData() {
							...
							for (let i = 0; i < res.data.list.length; i++) {
								...
								arr.unshift({
									...
									msg:{
										...
										type:"text",
										time:res.data.list[i].add_time,
										ps: res.data.list[i].ps,
										userinfo:{
											uid:res.data.list[i].users_id,
											username:res.data.list[i].nickname,
											face:res.data.list[i].avatar,
										},
										content:{
											text:res.data.list[i].message},
										}
								});
							}
							if (this.p==1){
			...
			sendMsg(content,type){
				...
				socket.send({
					...
					to_client_id:'all',
					room_id:this.roomid,
					to_client_name:this.user.userInfo.users_id,
					aliasname:this.user.userInfo.username+' '+this.user.wareRoom.villagename+' '+this.user.wareRoom.roomnumber,
				...
				let msg = {
					...
					msg:{
						...
						type:type,
						userinfo:{
							uid:this.user.userInfo.users_id,
							username:this.user.wareRoom.addr_id,
							face:this.user.userInfo.avatar,
						},
						content:content,
						},
					}
