...
function getLogin(type) {
	...
	//let url ="https://open.weixin.qq.com/connect/oauth2/authorize?appid=wxad277991b1e31287&redirect_uri=https://i.yodin.com/index.php/api/Index/getcode_gotoapp&response_type=code&scope=snsapi_userinfo&state=STATE#wechat_redirec";
	let url="http://i.yodin.com/index.php/index/Login/offi?offiurl=https://i.yodin.com/index.php/api/Index/getcode_gotoapp";
...
export const h5Login = function(type = "judge", callback) {
	var get_Request = getUrlData();
	var getRequest = {
		code:'',
		member_token:''
	};
	if (get_Request.query){
		getRequest = JSON.parse(decodeURIComponent(get_Request.query));
	}
	
	let recommendCode = getRecommendCode();
	console.log("调用h5Login",getRequest);
	...
	if (getBrowser() == "微信") {
		...
		}else if (getRequest.member_token) {
				api('user.tokenLogin', {
					token:getRequest.member_token,
				}).then(res => {
						if (res.code==200){
